...
For a general representation of state space $E$ (wave function space), the elements of $E$ are functions $\psi(\vec{\xi}),\ \phi(\vec{\xi})$, and the inner product in $E$ is defined as:
...
As the vectors of the basis $\{\phi_i\}$ of $E$ are orthonormal, we have that:
\begin{equation}
...
\begin{definition}
    ...
    \end{equation}
    And it constitutes a basis for the Hilbert space $E$ if every function in $E$ can be written as a linear combination of the functions of the basis in exactly one way:
    \begin{equation}
...
For a discrete orthonormal basis, we can express the scalar product in terms of the components as:
\begin{equation}
...
Therefore, the term in the parenthesis must be equal to $1$ for $\vec{\xi} =\vec{\xi}'$ and zero for every other case, so we obtain the \textbf{closure relation for a discrete orthonormal basis}:
\begin{equation}
