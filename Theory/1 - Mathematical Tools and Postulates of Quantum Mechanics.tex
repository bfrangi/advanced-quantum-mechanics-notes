...
Note that every set obeying these axioms is a complex vector space. For example, the set of $3 \times 2$ matrices with complex entries or the set of complex-valued continuous functions on $\R^4$ are both complex vector spaces.
...
Given any Hilbert space $\Hbt$, one can construct another complex vector space $\Hbt^*$, called the \textbf{dual vector space}. In order to understand these, we need to introduce the concept of \textbf{linear maps} and \textbf{linear functionals}.
...
Taking a step back, we know that all linear functionals in $\R^2$ take us from $\R^2$ to $\R^1$. Therefore, by definition, all linear functionals in $\R^2$ are represented by $1\times2$ matrices. In other words, the set of all linear functionals in $\R^2$ consists of the set of all row matrices. More generally, the set of all linear functionals in $\R^n$ consists of the set of all $1\times n$ row matrices. In fact, the set of all row matrices, much like column matrices, form their own vector space, which is known as the \textbf{dual space}.

This has been specific to $\R^n$ (particularly $\R^2$), but the dual space is a very general concept that is valid for abstract vector spaces too. More formally:
...
Notice that, when we put a bra and a ket together ($\bra{\phi}\ket{\psi}$), they look suspiciously like an inner product in this notation: $\braket{\phi}{\psi}$. Why have we not mentioned the inner product at all? If we go back at how our $L_x$ operator in $\R^2$ acts on a column vector:
...
We already defined linear operators in \textbf{Definition \ref{linear_map}}. In the quantum mechanical context, we can see them as entities that transform a ket into another ket. Some example linear operators are:
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item [e)] $\left(\ket{u}\bra{v}\right)^\dagger = \ket{v}\bra{u}$. \textit{Proof:} $\bra{\phi}\left(\ket{u}\bra{v}\right)^\dagger\ket{\psi} = \left[\bra{\psi}\left(\ket{u}\bra{v}\right)\ket{\phi}\right]^* = \braket{\psi}{u}^*\braket{v}{\phi}^* = \braket{u}{\psi}\braket{\phi}{v} = \braket{\phi}{v}\braket{u}{\psi} = \bra{\phi}\left(\ket{v}\bra{u}\right)\ket{\psi}$
    \end{enumerate}
...
Let's take a step back. So far, we have been talking about Hilbert spaces very generally, and we have not specified what a basis for one of these spaces might look like. Probably, the most intuitive basis could be a geometrical basis of vectors such as $\vec{e}_x = (1, 0, 0)$, $\vec{e}_y = (0, 1, 0)$ and $\vec{e}_z = (0, 0, 1)$. However, for quantum mechanics, we use \textit{functions} to define our bases. Each of the functions of a basis is a basic state in which the system can be found. Any possible state that the system could be in can be written as a linear combination of the functions of the basis. That is how we build up wave functions, which define the state of a system and belong to the \textbf{wave function space}.
