...
Then, we write the eigenvalue equation, assuming that $\vec{J}\,||\vec{B}$:% CHECK: We are computing the energy when the electrons are exposed to $\vec{B}$, which causes their spin angular momentum to align with the magnetic field. Why do we assume the whole angular momentum is aligned?
\begin{equation}
    \begin{split}
...
Take the simple example where $n=1$, where we have $l = 0$. The electron has spin $s=1/2$, so $m_s=\pm 1/2$. We also have two different values of $j$, which are $j = l+s = \frac12$ and $j = l - s = -\frac12$. Then: % CHECK: all this
\begin{itemize}
